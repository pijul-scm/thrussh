...
impl Session {
    ...
    pub fn server_read_encrypted<H: Handler>
        ...
        if buf[0] == msg::KEXINIT {
            ...
            if let Some(exchange) = enc.exchange.take() {
                let kexinit = KexInit::received_rekey(exchange,
        ...
        match state {
            ...
            Some(EncryptedState::WaitingAuthRequest(auth_request)) => {
                if buf[0] == msg::USERAUTH_REQUEST {
                    let auth_request = try!(enc.server_read_auth_request(handler,
                                                                         buf,
        ...
        match buf[0] {
            ...
            msg::CHANNEL_WINDOW_ADJUST => {
                ...
                Ok(Authenticated::future_unit(handler.window_adjusted(
                    channel_num, new_value as usize, self
                )))
            }
            ...
            msg::CHANNEL_REQUEST => {
                ...
                match req_type {
                    ...
                    x => {
                        ...
                        Ok(Authenticated::FutureUnit(FutureUnit::Done(futures::done(Ok((handler,
                                                                                        self))))))
                    }
                }
            }
    ...
    fn server_handle_channel_open<H: Handler>(mut self,
                                              handler: H,
                                              buf: &[u8])
            ...
            ReadAuthRequest::Req { ref mut future, ref mut auth_request } => {
                match *future {
                    ...
                    FutureAuth::Pubkey { ref mut validate, ref sig, ref init, ref key } => {
                        ...
                        let (handler, is_valid) = match *validate {
                            ...
                            ValidatePubkey::Future(ref mut h) => {
                                let (handler, auth) = try_ready!(h.poll()
                    ...
                    FutureAuth::PubkeyProbe { ref mut probe,
                        ...
                        let (handler, auth) = try_ready!(probe.poll()
                            .map_err(HandlerError::Handler));
                        if let Auth::Accept = auth {
